...
const fetchData = () => {
  let data = JSON.parse(localStorage.getItem("employee_transactions")) || []
  let index = data.findIndex((e) => e.id == router.currentRoute.value.params.id)
  if(index != -1){
    form.value = data[index]
    items.value = data[index]
    employee_name.value = getNameEmployee(data[index],employees.value,'id',"employee_id","khmer_name");      
  }
}
...
const onSubmit = () => {
    ...
    if(index != -1){
        ...
        localStorage.setItem('employee_transactions', JSON.stringify(data));
        Toast.fire({
...
<template>
    <AppForm
        btn-submit="Save"
        title="កែសម្រួលប្រតិបត្តិការបុគ្គលិក"
        @on-clear="resetForm"
        @submit="onSubmit"
    >
        <v-row>
